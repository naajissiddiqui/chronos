...
class JobSchedulerServiceTest {
    ...
    @Test
    void testActiveEnabledDueJobIsDetectedUpdatedAndCreatesOutboxEvent() {
        ...
        assertEquals("JOB_TRIGGERED", event.getEventType());
        assertEquals(job.getId(), event.getAggregateId());
        assertEquals(OutboxStatus.PENDING, event.getStatus());
        assertNull(event.getPublishedAt());
        assertTrue(event.getPayload().contains(job.getId().toString()));
        assertTrue(event.getPayload().contains(orgId.toString()));
    }

    @Test
    void testFutureJobIsIgnoredAndNoOutboxEventCreated() {
    ...
    @Test
    void testDisabledJobIsIgnoredAndNoOutboxEventCreated() {
        Instant past = Instant.now().minus(5, ChronoUnit.MINUTES);
    ...
    @Test
    void testPausedJobIsIgnoredAndNoOutboxEventCreated() {
        Instant past = Instant.now().minus(5, ChronoUnit.MINUTES);
    ...
    @Test
    void testMultipleDueJobsCreateMultipleOutboxEvents() {
        ...
        createJob("Job 2", JobStatus.ACTIVE, true, "*/10 * * * *", "UTC", past2);
    ...
    @Test
    void testMissedScheduleMovesToNextFutureOccurrenceAndCreatesOutboxEvent() {
    ...
    @Test
    void testRepeatedPollingDoesNotCreateDuplicateOutboxEvents() {
        Instant past = Instant.now().minus(5, ChronoUnit.MINUTES);
        ...
        // Immediate second poll should find 0 due jobs and create no further outbox events
    ...
    @Test
    void testAtomicUpdateFailsIfOldNextRunAtMismatchedDoesNotCreateOutboxEvent() {
        Instant past = Instant.now().minus(5, ChronoUnit.MINUTES);
